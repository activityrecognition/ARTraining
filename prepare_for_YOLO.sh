...
python video_downloader.py -e $email -g '["'"$group_name"'"]' -t '["15_tim"]' -o $output_videos -l '[]' --incremental
...
sorted_frame_list=/media/Gui2/thermix/ARThermal/"$group_name"_frames_no_movement/15_tim/sorted_frames.txt

# list frame paths in a file excluding black frames
output_file="$sorted_frame_list" \
dataset_dir=/media/Gui2/thermix/ARThermal/"$group_name"_frames_no_movement \
tim=15_tim \
category=3 \
